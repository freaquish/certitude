...
def migrate_posts():
    ...
    for post in posts.iterator():
        v_stores: QuerySet = action_stores.filter(Q(post_id=post.post_id) & Q(viewed=True)).values_list('account_id', flat=True)
        ...
        post.score = Analyzer(user=None).score_post({'view': post.views.count(), 'love': post.loves.count(), 'share': post.shares.count()})
